...
def mock_create_user(mocker):
    return mocker.patch("app.crud.create_user", return_value={"id": 1, "name": "Anuraag Mishra"})
...
def mock_transfer_money(mocker):
    return mocker.patch("app.crud.transfer_money", return_value={"amount": 50.0, "from_account_id": 1, "to_account_id": 2})
...
def test_create_user_route(mock_create_user):
    response = client.post("/users/?name=Anuraag Mishra")
...
def test_transfer_money_route(mock_transfer_money):
    response = client.post("/users/?name=Anuraag Mishra")
    response = client.post("/accounts/1/?initial_balance=1000")
    response = client.post("/accounts/1/?initial_balance=1000")
